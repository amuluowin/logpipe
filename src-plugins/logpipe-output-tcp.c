...
int InitOutputPluginContext(struct LogpipeEnv *p_env, struct LogpipeOutputPlugin *p_logpipe_output_plugin, void *p_context)
{
	...
	{
		int ret = getaddrinfo(p_forward_session->ip, NULL, &hints, &result);
		if (ret != 0)
		{
			ERRORLOGC("getaddrinfo failed! error %d : %s host:[%s]", ret, gai_strerror(ret), p_forward_session->ip);
			return -1;
		}
		...
		for (ptr = result; ptr != NULL; ptr = ptr->ai_next)
		{
			...
			p_forward_session->ip = (char *)inet_ntop(AF_INET, &addr->sin_addr, ipbuf, sizeof(ipbuf));
...
_GOTO_RETRY_SEND:
	...
	if (len == -1)
	{
		...
		close(p_plugin_ctx->p_forward_session->sock);
		p_plugin_ctx->p_forward_session->sock = -1;
...
int AfterWriteOutputPlugin(struct LogpipeEnv *p_env, struct LogpipeOutputPlugin *p_logpipe_output_plugin, void *p_context, uint16_t filename_len, char *filename)
{
	...
	INFOLOGC("SEND-FILENAME[%ld.%06ld] SEND-BLOCK-LEN[%ld.%06ld] SEND-BLOCK[%ld.%06ld] SEND-EOB[%ld.%06ld]", tv_diff_send_filename.tv_sec, tv_diff_send_filename.tv_usec, tv_diff_send_block_len.tv_sec, tv_diff_send_block_len.tv_usec, tv_diff_send_block.tv_sec, tv_diff_send_block.tv_usec, tv_diff_send_eob.tv_sec, tv_diff_send_eob.tv_usec)
